...
import com.daiyan.handwork.app.MyApplication;
import com.daiyan.handwork.common.ImageSizeUtil.ImageSize;
import com.daiyan.handwork.constant.Consts;
import com.daiyan.handwork.utils.BitmapUtils;
import com.daiyan.handwork.utils.DownloadImgUtils;
			...
			mUIHandler = new Handler()
			{
				public void handleMessage(Message msg)
				{
					...
					if (imageview.getTag().toString().equals(path))
					{
						if(bm != null) {
							// 防止图片太大爆内存
							if(bm.getWidth() > Consts.MAX_IMG_SIZE) {
								int width = Consts.MAX_IMG_SIZE;
								int height = (bm.getHeight() * width) / bm.getWidth();
								bm = BitmapUtils.zoomBitmap(bm, width, height);
							}
...
//							Log.e(TAG, "load image :" + path + " to memory.");
	...
	private Runnable getTask()
	{
		if (mTaskQueue.isEmpty())
		{
			...
			map.put("version", SystemUtils.getVersion(context));
			map.put("phoneInfo", "设备厂商: " + android.os.Build.BRAND + 
					"型号: " + android.os.Build.MODEL + ", SDK: "
	                + android.os.Build.VERSION.SDK + ", "
	                + android.os.Build.VERSION.RELEASE);
			MobclickAgent.onEvent(context, "ImgLoaderTaskQueueEmptyError", map); 
			return null;
		}
